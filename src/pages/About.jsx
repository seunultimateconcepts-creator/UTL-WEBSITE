...
function About() {
  ...
  // ✅ Core values
  const values = [
    { icon: Gem, title: 'Excellence', desc: 'We deliver nothing short of the best in everything we do.' },
    { icon: ShieldCheck, title: 'Integrity', desc: 'Honest, transparent and trustworthy in all our dealings.' },
    { icon: Zap, title: 'Innovation', desc: 'Always using the latest technology to solve problems.' },
    { icon: Handshake, title: 'Partnership', desc: 'We treat every client as a long-term partner not just a customer.' },
    { icon: Globe, title: 'Global Mindset', desc: 'Built for Nigeria but designed to serve the world.' },
    { icon: TrendingUp, title: 'Growth', desc: 'We are committed to growing with our clients every step of the way.' },
  ]
  ...
  // ✅ Sub-page links
  const subPages = [
    { title: 'Who Are We', desc: 'Our full story and background', path: '/about/who-we-are', icon: Users },
    { title: 'Values & Updates', desc: 'What we stand for and latest news', path: '/about/values', icon: Gem },
    { title: 'Our Vision', desc: 'Where we are headed', path: '/about/vision', icon: Target },
    { title: 'FAQs', desc: 'Answers to common questions', path: '/about/faqs', icon: HelpCircle },
  ]
  ...
  return (
    <div className="pt-16">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6">
              <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-full px-4 py-1.5">
                ...
                <span className="text-amber-300 text-xs font-medium tracking-wide">ABOUT US</span>
              </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <p className="text-amber-600 text-sm font-semibold tracking-widest uppercase mb-3">THE FOUNDER</p>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center max-w-5xl mx-auto">
            ...
            <div className="flex justify-center">
              <div className="relative">
                <div className="absolute inset-0 bg-amber-500/20 rounded-3xl blur-2xl scale-110" />
                {founder.image ? (
                  <img
                    src={founder.image}
                    alt={founder.name}
                  ...
                  <div className="relative w-72 h-72 bg-gradient-to-br from-[#0a0f2c] to-amber-600 rounded-3xl shadow-2xl flex items-center justify-center">
                    <UserRound size={96} className="text-white/80" />
                  </div>
                )}
                {/* Online badge */}
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-amber-600 font-semibold">{founder.role}</p>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {founder.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1.5 bg-amber-50 text-amber-700 text-xs font-semibold rounded-lg border border-amber-100"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="flex gap-3">
                  {founder.socials.map((social) => (
                    <a
                      ...
                      title={social.name}
                      className="w-10 h-10 rounded-xl bg-gray-100 hover:bg-amber-500 hover:text-white flex items-center justify-center text-gray-500 transition-all"
                    >
                      <social.icon size={17} />
                    </a>
                  ))}
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <p className="text-amber-600 text-sm font-semibold tracking-widest uppercase mb-3">WHAT WE STAND FOR</p>
            ...
            {values.map((value) => (
              ...
              >
                <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center mb-4 group-hover:bg-amber-100 transition-colors">
                  <value.icon size={22} className="text-amber-600" />
                </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {/* Vertical line */}
            <div className="absolute left-8 top-0 bottom-0 w-px bg-amber-100 hidden md:block" />

            <div className="space-y-8">
              {timeline.map((item,) => (
                <div key={item.year} className="flex gap-6 items-start">
                  {/* Year circle */}
                  <div className="flex-shrink-0 w-16 h-16 bg-[#0a0f2c] rounded-2xl flex items-center justify-center text-amber-400 font-black text-sm relative z-10">
                    {item.year}
                  </div>
                  {/* Content */}
                  <div className="flex-1 bg-gray-50 rounded-2xl p-5 border border-gray-100 hover:border-amber-200 hover:shadow-md transition-all">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {subPages.map((item) => (
              ...
              >
                ...
                <p className="text-amber-600 text-sm font-semibold mt-3 group-hover:gap-2 flex items-center justify-center gap-1 transition-all">
